...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        var progressName = $("<td/>");
        progressName.append('<p class="col-center text-green">'+file.name+'</p>');

        var progressText = $("<td/>");
        //progressText.addClass('progressName').text(file.name); // 文件名
...
//console.log(up,info);
    ...
    if (res.url) {
        url = res.url;
        str  = "<div><a href=" + res.url + " target='_blank' ><span class='badge bg-green'>100%</span> 查看</a> <a href='javascript:delattach(\"" + res.key + "\"," + type + ");'> 删除</a></div>";
        //str += "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    } else {
        var domain = up.getOption('domain');
        url = domain + encodeURI(res.key);
        var link = domain + res.key;
        str  = "<div><a title='查看原图' href=" + url + " target='_blank' ><span class='badge bg-green'>100%</span> 查看</a> <a href='javascript:delattach(\"" + res.key + "\"," + type + ");'> 删除</a></div>";
        //str += "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
...
//console.log(url);
            ...
            linkWrapper.attr('href', linkUrl).attr('title', '查看原图');
